...
| **Alt** | **忽略字形**：跳過台灣常用異體字轉換 (TWVariants)、台灣慣用詞字形修正 (TWVariantsPhrases)、CJK 相容字形修正。 |
| **Shift** | **純淨模式**：僅使用標準詞組與單字，忽略視界邏輯、語法邏輯、自訂擴充、台灣字形修正及例外清單。純淨模式測試顯示，在 190 萬行、約 5000 萬字元、900 萬處轉換的規模下，包含 CJK 相容字處理在內的繁簡 (T2S/S2T) 轉換結果，與 OpenCC 官方完全相同。|
...
* **視界邏輯探測引擎 (Vision Logic) / 视界逻辑探测引擎**：
  ...
  * 語法：ContextLogicData = { "單字元金鑰": [ "條件判斷式 1 => 目標 1", "條件判斷式 2 => 目標 2", "default => 預設目標" ] };
    - 从第一出口驶出 ➡️ 從第一出口駛出 ； 喷发胶 ➡️ 噴髮膠

* **詞彙邏輯語境導正 (Phrase Logic) / 词汇逻辑语境导正**：
  * **多維度前後文標籤判定**：系統將複雜的短語規則解析為「指定包含詞」與「排除詞」斷言（支援 !排除詞 的右/左權重過濾），在不破壞基礎分詞架構的前提下，利用自研雙向語境特徵捕捉，藉由前後各擴展 6 字的精確視野視窗，對可能產生歧義的連續字塊進行「多層次標籤比對」，導正特定詞彙或高歧義句型。並引入「@ 定向定錨機制」精確比對目標左右緊鄰的詞彙。此外，在 PhraseLogic 滑動視窗探索中新增「標點符號防火牆」機制，雙端皆內建標點字元（如 。，！\n）阻斷映射與長度門檻限制。
  * 語法：PhraseLogic = {"關鍵詞": "註解 替換目標 [左包含|!左排除] [右包含|!右排除] {視窗大小,標點穿越[0阻斷/1穿越],長度門檻}"}
...
* **CJK 相容字處理(Compatibility Ideographs Normalization) / CJK 相容字处理**:
  0.43 版起，C# 與 v8 端皆建立靜態相容字映射表（FastReplaceMap），鎖定 U+F900 ~ U+FAFF 區間與平面 2 的高位代理字。一旦偵測到相容字，即在執行分割前原地重組字串進行正規化，確保輸入內碼與標準字庫對齊。解決相容字與標準字內碼相異導致詞條無法命中的問題。
