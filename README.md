...
| **DllConvert.dll** (Optional) | N/A  | **C# 增壓引擎 (Optional)**：處理超大型文本(>3,000萬字)的快速轉換核心。若無下載此檔，系統會自動執行原本的純 JS 優化邏輯，不影響原功能。 / **C# 增压引擎 (Optional)**：处理超大型文本(>3,000万字)的快速转换核心。若无下载此档，系统会自动执行原本的纯 JS 优化逻辑，不影响原功能。 (**需 EmEditor v26.1+**) |
| **DllConvert.cs** (C#) | N/A  | **增壓引擎原始碼**：DllConvert.dll 的 C# 原始程式碼，內含編譯指令與專案資訊，供開發者自行編譯或功能修改。 / **增压引擎源码**：DllConvert.dll 的 C# 原始源代码，内含编译指令与项目信息，供开发者自行编译或功能修改。 |
| **DllConvert.jsee**  (Optional) | N/A | **中介加速腳本**：負責智慧分段 (Chunking) 並透過 PowerShell 呼叫 DLL 進行增壓轉換。 / **中介加速脚本**：负责智慧分段 (Chunking) 并通过 PowerShell 调用 DLL 进行增压转换。 (**需 EmEditor v26.1+**)|
...
| **T2S.jsee + DllConvert** | 點選 **簡** 巨集 | **約 20 秒** | 搭配 DllConvert，需 EmEditor v26.1+。 ||
